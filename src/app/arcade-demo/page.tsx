...
export default function ArcadeDemoPage() {
  ...
  useEffect(() => {
    ...
    const notes = parseMusicXMLToArcadeNotes(xmlData, 120);
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center py-12 px-4">
      <div className="max-w-4xl w-full flex items-center justify-between mb-8">
        <div>
          <h1 className="text-4xl font-black text-slate-800 tracking-tight">Piano Arcade <span className="text-blue-500">BETA</span></h1>
          <p className="text-slate-500 mt-2">Leyendo XML dinámico: <strong>{sightReadingExercises[exerciseIndex].title}</strong></p>
        </div>
      ...
      <div className="w-full max-w-5xl bg-white p-8 rounded-3xl shadow-sm border border-slate-200 mx-auto flex flex-col items-center">
        <div className="w-full mb-8 flex flex-col md:flex-row gap-4">
          <div className="flex-1 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <ul className="text-amber-700 text-sm list-disc pl-5 space-y-1">
              <li>Si tienes un <strong>Piano MIDI USB</strong> conectado, úsalo directamente.</li>
              <li>Si no tienes piano, usa tu <strong>teclado de computadora</strong>.</li>
              <li><strong>Teclas Mapeadas (C4 a C5):</strong> A=Do, S=Re, D=Mi, F=Fa, G=Sol, H=La, J=Si, K=Do (agudo).</li>
            </ul>
          </div>
            ...
            <div className="flex flex-col gap-4">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  ...
                  <div className="text-xs text-blue-700">El motor se detendrá y te esperará si no tocas la nota a tiempo.</div>
                </div>
              </label>
        ...
        {gameState === "finished" && finalResult ? (
          <div className="bg-white rounded-3xl border-4 border-emerald-500 p-12 flex flex-col items-center justify-center shadow-2xl animate-in fade-in zoom-in duration-500 max-w-lg w-full">
            ...
            <p className="text-xl text-slate-500 mb-8 font-medium">Ejercicio de lectura a primera vista</p>
