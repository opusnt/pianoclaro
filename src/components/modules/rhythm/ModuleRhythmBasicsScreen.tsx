...
type ModuleRhythmBasicsScreenProps = {
  module: RhythmModule;
};

export function ModuleRhythmBasicsScreen({ module }: ModuleRhythmBasicsScreenProps) {
  const { progress, saveAttempt, resetProgress } = useExerciseProgress(module.id, module.exercises);
  ...
  useEffect(() => {
    trackEvent("rhythm_module_started", {
      moduleId: module.id,
  ...
  function handleAttemptComplete(attempt: ExerciseAttempt) {
    saveAttempt(attempt);
    trackAttempt(module.id, attempt);

    if (attempt.passed && attempt.exerciseId === module.exercises.at(-1)?.id) {
      trackEvent("rhythm_module_completed", {
        moduleId: module.id,
      ...
      <div className="mx-auto max-w-7xl">
        <section className="rounded-2xl border border-blue-deep/10 bg-white/85 p-6 shadow-soft sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-bold uppercase text-gold-soft">Módulo 2</p>
              <h1 className="mt-2 text-4xl font-bold text-blue-deep sm:text-5xl">
                {module.title}
              </h1>
              <p className="mt-2 text-lg font-semibold text-muted">{module.subtitle}</p>
