...
export function PentatonicExerciseScreen({
  ...
  const currentProgressUnits = getCompletedUnits({
    ...
    currentPlayedNotes:
      question?.mode === "improvisation" ? (engine.currentAnswer ? ["improv"] : []) : engine.currentPlayedNotes,
      ...
      <div className="mt-5 grid gap-5 min-[1800px]:grid-cols-[minmax(0,1fr)_minmax(21rem,24rem)]">
        <div className="min-w-0 space-y-4">
          ...
          <PentatonicKeyboard
            ...
            disabled={engine.state !== "active" || Boolean(question?.answerOptions) || (!isImprovisation && engine.questionComplete)}
          ...
          {isImprovisation ? (
            <ImprovisationPanel
              ...
              uniqueNotes={new Set(engine.improvisedNotes.map((note) => note.replace(/\d/g, ""))).size}
              disabled={engine.state !== "active" || Boolean(engine.currentAnswer)}
    ...
    <div className="rounded-2xl border border-blue-deep/10 bg-ivory p-5">
      ...
      {question && scale ? (
        <div className="mt-4 grid gap-3 text-sm font-semibold text-muted sm:grid-cols-3">
          <p><span className="font-bold text-blue-deep">Escala:</span> {scale.displayName}</p>
          <p><span className="font-bold text-blue-deep">Tipo:</span> {scale.type === "major" ? "mayor" : "menor"}</p>
          <p><span className="font-bold text-blue-deep">Tónica:</span> {getDisplayPitchName(scale.tonic)}</p>
        </div>
      ) : null}
    </div>
  );
...
function ScaleSummary({ scaleId }: { scaleId: string }) {
  ...
  return (
    <div className="rounded-2xl border border-blue-deep/10 bg-ivory p-4">
      ...
      <p className="mt-2 text-sm font-semibold leading-6 text-muted">{getPentatonicDisplaySequence(scale)}</p>
