...
import type { IntervalAnswer } from "@/types/intervals";

type IntervalFeedbackProps = {
  message: string;
  answer?: IntervalAnswer | null;
};

export function IntervalFeedback({ message, answer }: IntervalFeedbackProps) {
      ...
      {answer ? (
        <dl className="mt-3 grid gap-2 text-xs font-semibold sm:grid-cols-2">
          <div>
            ...
            <dd className="mt-1">{answer.expectedAnswer}</dd>
          </div>
          <div>
            <dt className="uppercase opacity-70">Respuesta</dt>
            <dd className="mt-1">{answer.userAnswer}</dd>
          </div>
        </dl>
      ) : null}
    </div>
