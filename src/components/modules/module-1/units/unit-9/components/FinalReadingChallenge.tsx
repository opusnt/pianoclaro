...
export function FinalReadingChallenge({ onComplete }: FinalReadingChallengeProps) {
  ...
  useEffect(() => {
    if (gameState === "playing" && arcadeNotes.length === 0) {
      try {
        const parsed = parseMusicXMLToArcadeNotes(odeToJoyXml, 100);
      ...
      {gameState === "intro" && (
        <div className="bg-white p-8 md:p-12 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center animate-in fade-in zoom-in duration-500">
          ...
          <h2 className="text-3xl font-black text-slate-800 mb-4 text-center">
            Examen Final: Himno a la Alegría
          </h2>
          <p className="text-slate-500 text-center mb-8 max-w-md">
            Es hora de poner a prueba todo lo que has aprendido. Tocarás tu primera canción real con
            ambas manos (bueno, por ahora solo la melodía). El motor avanzará a tu propio ritmo
      ...
      {gameState === "playing" && (
        <div className="w-full relative">
          <div className="absolute -top-12 left-0 right-0 flex justify-between items-center px-4">
            <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-bold tracking-widest uppercase">
              Modo Examen
            </span>
