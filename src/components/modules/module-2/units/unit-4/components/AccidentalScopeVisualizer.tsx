...
export function AccidentalScopeVisualizer({
    ...
    return {
      ...
      color:
        activeStepIndex === index
          ? "bg-sky-400 text-slate-900 shadow-sky-200"
    ...
    for (let i = 0; i < exerciseNotes.length; i++) {
      ...
      if (note.measure > currentMeasure) {
        if (activeSharpFlat) {
          ...
          const sharpX = activeSharpFlat.x;
          const barlineX = barLines.find((b) => b < (vNote.xPos || 0) && b > sharpX);
          if (barlineX) {
            scopes.push({
      ...
      if (note.accidental === "sharp" || note.accidental === "flat") {
        ...
        activeSharpFlat = {
          ...
          noteBase: note.note[0],
        };
      } else if (
        note.accidental === "natural" &&
        activeSharpFlat &&
        note.note[0] === activeSharpFlat.noteBase
      ) {
        scopes.push({
          startX: activeSharpFlat.x,
          endX: vNote.xPos || 0,
  ...
  return (
    <div className="relative w-full">
      <TrebleClefVisualizer notes={visualNotes} barLines={barLines} interactive={false} />
