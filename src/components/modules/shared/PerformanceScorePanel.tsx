...
export function PerformanceScorePanel({
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <Metric icon={<Target className="h-4 w-4" />} label="Accuracy" value={`${Math.round(accuracy * 100)}%`} />
      ...
      <Metric icon={<Gauge className="h-4 w-4" />} label={finalMetricLabel} value={finalMetricValue} />
    </div>
  );
}
